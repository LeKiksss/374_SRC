// NEG instruction testbench: neg R4, R7 — T3 R7out,NEG,Zin; T4 Zlowout,R4in (no Y load)
`timescale 1ns/10ps
module tb_neg;
    reg Clock, Clear;
    reg [15:0] Rin, Rout;
    ...
    parameter Default=4'b0000, Reg_load1a=4'b0001, Reg_load1b=4'b0010, T0=4'b0111, T1=4'b1000, T2=4'b1001, T3=4'b1010, T4=4'b1011;
    ...
    always @(Present_state) begin
        ...
        Mdatain=0;
        case (Present_state)
            Reg_load1a: begin Mdatain=32'hFFFF_FFFB; Read=1; MDRin=1; end  // R7 <- -5 (0xFFFFFFFB)
            Reg_load1b: begin MDRout=1; Rin[7]=1; end
            T0: begin PCout=1; MARin=1; IncPC=1; Zin=1; end
            T1: begin Zlowout=1; PCin=1; Read=1; MDRin=1; Mdatain=32'h11840000; end  // neg R4,R7
            T2: begin MDRout=1; IRin=1; end
            T3: begin Rout[7]=1; NEG=1; Zin=1; end   // B=R7 on bus, NEG -> Z = -B
            T4: begin Zlowout=1; Rin[4]=1; end       // R4 <- neg(R7) = 5
        endcase
    end
endmodule
