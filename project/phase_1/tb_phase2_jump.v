...
module tb_phase2_jump;

    localparam LABEL_W = 8*48;
    localparam [4:0] OP_JR  = 5'h07;
    localparam [4:0] OP_JAL = 5'h08;

    localparam [4:0]
        S_CLEAR_1 = 5'd0, S_T0_1 = 5'd1, S_T1_1 = 5'd2, S_T2_1 = 5'd3, S_T3_1 = 5'd4,
        S_CLEAR_2 = 5'd5, S_T0_2 = 5'd6, S_T1_2 = 5'd7, S_T2_2 = 5'd8, S_T3_2 = 5'd9, S_T4_2 = 5'd10,
        S_DONE    = 5'd11;

    reg  [4:0]  Present_state;
    reg         Clock, Clear;
    reg         Gra, Grc, Rin, Rout;
    reg         PCin, IRin, MARin, Zin, MDRin;
    reg         IncPC, Read;
    reg         PCout, MDRout, Zlowout;
    ...
    Datapath_top DUT (
        .Clock(Clock), .Clear(Clear), .Gra(Gra), .Grb(1'b0), .Grc(Grc), .Rin(Rin), .Rout(Rout), .BAout(1'b0),
    ...
    function [31:0] encode_jal;
        ...
        input [3:0] rc;
        begin
            encode_jal = {opcode, ra, 4'h0, rc, 15'b0};
        end
    endfunction
    ...
    always @(*) begin
        Clear = 0;
        Gra = 0; Grc = 0; Rin = 0; Rout = 0;
        PCin = 0; IRin = 0; MARin = 0; Zin = 0; MDRin = 0;
        IncPC = 0; Read = 0;
        PCout = 0; MDRout = 0; Zlowout = 0;
        ...
        case (Present_state)
            ...
            S_T3_1: begin Gra = 1; Rout = 1; PCin = 1; end
            S_T3_2: begin PCout = 1; Grc = 1; Rin = 1; end
            S_T4_2: begin Gra = 1; Rout = 1; PCin = 1; end
        endcase
    end
    ...
    always @(posedge Clock) begin
        ...
        case (Present_state)
            ...
            S_T3_1: begin
                expect32("jr R12", PC, 32'h0000_00FF);
            ...
            S_T4_2: begin
                ...
                if (failures == 0) begin
                    ...
                    $display("FAIL: GROUP 4 completed with %0d failure(s)", failures);
                end
