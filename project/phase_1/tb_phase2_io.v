...
module tb_phase2_io;
    ...
    reg         Clock, Clear;
    reg         Gra, Rin, Rout;
    reg         PCin, IRin, MARin, Zin, MDRin, Out_Portin;
    reg         IncPC, Read;
    reg         PCout, MDRout, Zlowout, In_Portout;
    reg [31:0]  port_in;
    ...
    Datapath_top DUT (
        .Clock(Clock), .Clear(Clear), .Gra(Gra), .Grb(1'b0), .Grc(1'b0), .Rin(Rin), .Rout(Rout), .BAout(1'b0),
        .PCin(PCin), .IRin(IRin), .Yin(1'b0), .MARin(MARin), .HIin(1'b0), .LOin(1'b0), .Zin(Zin), .MDRin(MDRin),
        .CONin(1'b0), .Out_Portin(Out_Portin), .R12in_force(1'b0), .IncPC(IncPC), .Read(Read), .Write(1'b0), .PCout(PCout),
        .MDRout(MDRout), .HIout(1'b0), .LOout(1'b0), .Zhighout(1'b0), .Zlowout(Zlowout),
        .In_Portout(In_Portout), .Cout(1'b0), .AND(1'b0), .OR(1'b0), .NOT_op(1'b0), .NEG(1'b0), .SHR(1'b0),
    ...
    always @(*) begin
        Clear = 0;
        Gra = 0; Rin = 0; Rout = 0;
        PCin = 0; IRin = 0; MARin = 0; Zin = 0; MDRin = 0; Out_Portin = 0;
        IncPC = 0; Read = 0;
        PCout = 0; MDRout = 0; Zlowout = 0; In_Portout = 0;
    ...
    always @(posedge Clock) begin
        ...
        case (Present_state)
            S_CLEAR_1: begin
                ...
                print_memory_contents("out R7 setup", 9'h000);
            ...
            S_CLEAR_2: begin
                ...
                DUT.U_DP.PC_reg.q = 32'h0000_0000;
                print_memory_contents("in R5 setup", 9'h000);
